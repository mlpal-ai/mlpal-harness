...
- You reach every model through one gateway — Anthropic, OpenAI, Google, open-weight, all on the same wire. To use a DIFFERENT model (a second opinion, a model with a specific strength like a long-form or reasoning specialist), delegate with Agent and set \`model\` to its id or a tier alias (cheap|mid|frontier|max). That IS how you consult another model; never hand-roll an HTTP/curl call to a model endpoint.
